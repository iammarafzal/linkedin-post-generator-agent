...
# Omit temperature parameter for flash-lite to eliminate warning logs
GENERATOR_MODEL = os.getenv("GENERATOR_MODEL", gemini_3_5_flash_lite)
generator_llm = ChatGoogleGenerativeAI(model=GENERATOR_MODEL)
structured_generator_llm = generator_llm.with_structured_output(
    LinkedInPostSchema
)

# 2. Evaluator LLM: Fast structured rule evaluation
EVALUATOR_MODEL = os.getenv("EVALUATOR_MODEL", gemini_2_5_flash_lite)
evaluator_llm = ChatGoogleGenerativeAI(model=EVALUATOR_MODEL)
...
optimizer_llm = ChatGoogleGenerativeAI(model=OPTIMIZER_MODEL)